...
export default (db) => {
  ...
  const handleOrderCreated = async (orderData) => {
    ...
    if (!localRestaurant) {
      console.warn(
        `Restaurante com ID ${restaurantId} do Uai Rango não encontrado no sistema.`,
      );
      return;
    }
  ...
  const handleOrderUpdated = async (orderData) => {
    const order = await models.Order.findOne({
      where: { external_order_id: orderData.id, platform: "uai_rango" },
    });
    if (order) {
      await order.update({ status: orderData.status });
    } else {
      console.warn(
        `Pedido Uai Rango ${orderData.id} não encontrado para atualização de status.`,
      );
    }
  };

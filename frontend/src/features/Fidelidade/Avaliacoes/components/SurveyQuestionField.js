  ...
  return (
    <Paper elevation={1} sx={{ p: 2, mb: 2, borderLeft: '4px solid #1976d2' }}>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
        <Typography variant="h6">{t('survey_create.question_title', { num: questionIndex + 1 })}</Typography>
        ...
        render={({ field }) => (
          <FormControl fullWidth margin="normal" error={!!errors.questions?.[questionIndex]?.type}>
            ...
            <Select {...field} label={t('survey_create.question_type_label')}>
              ...
              <MenuItem value="single-choice">{t('survey_create.question_type_single_choice')}</MenuItem>
              <MenuItem value="multiple-choice">{t('survey_create.question_type_multiple_choice')}</MenuItem>
            </Select>
        ...
        <Box mt={2}>
          ...
          {optionsFields.map((item, optionIndex) => (
            <Box key={item.id} display="flex" alignItems="center" gap={1} mb={1}>
              ...
              />
              <IconButton onClick={() => removeOption(optionIndex)} color="error" aria-label={t('common.delete')}>
                <DeleteIcon />
              </IconButton>
            </Box>
          ))}

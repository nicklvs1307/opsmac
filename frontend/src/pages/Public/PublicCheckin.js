  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (data.coupon_code) {
        try {
          ...
          if (couponValidationResponse.data.is_valid) {
            ...
            toast.success(t('public_checkin.coupon_applied_success'));
          } else {
            toast.error(t('public_checkin.coupon_invalid_or_expired'));
            setLoading(false);
            return; // Stop submission if coupon is invalid
          }
        } catch (couponError) {
      ...
      const payload = {
        ...
        coupon_id: couponId, // Include validated coupon ID in payload
      };

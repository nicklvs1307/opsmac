...
export default (db) => {
  ...
  const processIncomingMessageInternal = async (message, restaurant) => {
    ...
    let customer = null;
    const restaurantIdForCustomer =
      recentFeedbackRequest?.restaurant_id || restaurant.id;
    ...
    if (recentFeedbackRequest) {
      await processAutomaticFeedbackResponseInternal(
        message,
        customer,
        recentFeedbackRequest,
        restaurant,
      );
    }
  ...
  const processAutomaticFeedbackResponseInternal = async (
    message,
    customer,
    feedbackRequest,
    restaurant,
  ...
  const sendFeedbackRequestInternal = async (
    restaurant,
    phone_number,
    customer_name,
    table_number,
  ...
  const sendBulkFeedbackInternal = async (
    restaurant,
    recipients,
    custom_message,
    userId,
    ...
    for (const recipient of recipients) {
      try {
        ...
        if (whatsappResponse.success) {
          results.sent.push({
            phone_number,
            whatsapp_message_id: whatsappResponse.data?.id,
          });
        } else {
  ...
  const sendManualMessageInternal = async (
    restaurant,
    ...
    if (!whatsappResponse.success) {
      throw new Error(
        whatsappResponse.error || "Erro ao enviar mensagem manual",
      );
    }
  ...
  const listMessagesInternal = async (
    restaurantId,
    page = 1,
    limit = 20,
    status,
  ...
  const getWhatsappAnalyticsInternal = async (restaurantId, period = "30d") => {
    ...
    const [messageStats, deliveryStats, responseStats, totalSent] =
      await Promise.all([
    ...
    const conversionRate =
      totalSent > 0 ? (responseStats / totalSent) * 100 : 0;
  ...
  const processIncomingMessage = async (messageBody) => {
    ...
    if (restaurant) {
      ...
      console.warn(
        "Mensagem recebida de instância não mapeada:",
  ...
  const sendFeedbackRequest = async (restaurantId, data, userId) => {
    ...
    return await sendFeedbackRequestInternal(
      restaurant,
      phone_number,
      customer_name,
      table_number,
  ...
  const sendBulkFeedback = async (restaurantId, data, userId) => {
    ...
    const restaurant = await models.Restaurant.findByPk(restaurantId);
    if (!restaurant) throw new NotFoundError("Restaurante não encontrado");
    return await sendBulkFeedbackInternal(
      restaurant,
      recipients,
      custom_message,
      userId,
  ...
  const sendManualMessage = async (restaurantId, data, userId) => {
    ...
    const restaurant = await models.Restaurant.findByPk(restaurantId);
    if (!restaurant) throw new NotFoundError("Restaurante não encontrado");
    return await sendManualMessageInternal(
      restaurant,
      recipient_phone_number,
      message_text,
      userId,
  ...
  const listMessages = async (restaurantId, queryParams) => {
    return await listMessagesInternal(
      restaurantId,
      queryParams.page,
      queryParams.limit,
      queryParams.status,

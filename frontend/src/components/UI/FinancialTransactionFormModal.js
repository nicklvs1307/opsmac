...
const FinancialTransactionFormModal = ({
  open,
  handleClose,
  handleSave,
  restaurantId,
  ...
  const [recurringEndsAt, setRecurringEndsAt] = useState(
    initialData?.recurring_ends_at
      ? new Date(initialData.recurring_ends_at).toISOString().split('T')[0]
      : ''
  );
  ...
  } = useQuery(['financialCategories', restaurantId, type], fetchFinancialCategories, {
    ...
    >
      <Box sx={style}>
        ...
        <Typography id="transaction-form-modal-title" variant="h6" component="h2" gutterBottom>
          {initialData
            ? t('financial.edit_transaction_title')
            : t('financial.add_transaction_title')}
        </Typography>

...
const checkSubscription = async (req, res, next) => {
  try {
    const restaurantId =
      req.params.restaurantId || req.body.restaurant_id || req.restaurant?.id;
    ...
    const restaurant =
      req.restaurant || (await models.Restaurant.findByPk(restaurantId));
    ...
    if (!restaurant.isSubscriptionActive()) {
      return res.status(402).json({
        error:
          "Subscription expirada. Renove seu plano para continuar usando o serviço",

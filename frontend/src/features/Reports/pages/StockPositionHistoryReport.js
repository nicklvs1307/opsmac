...
import { useStockPositionHistoryReport, useStockItemsReport } from '@/features/Reports/api/financialReportsService';
...
const StockPositionHistoryReport = () => {
  ...
  const {
    ...
    isError,
    refetch,
  } = useStockPositionHistoryReport(restaurantId, filters, {
    onError: (error) => {
      toast.error(
  ...
  } = useStockItemsReport(restaurantId, {
    onError: (error) => {
      toast.error(

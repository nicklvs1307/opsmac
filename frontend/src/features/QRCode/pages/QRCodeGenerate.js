...
import { useGenerateQRCode, useDownloadQRCode, usePrintQRCode } from './api/qrCodeService';
...
const QRCodeGenerate = () => {
  ...
  const downloadQRCodeMutation = useDownloadQRCode({
    onError: (err) => {
      toast.error(err.response?.data?.message || 'Erro ao baixar QR Code');
    },
  });

  const printQRCodeMutation = usePrintQRCode({
    onError: (err) => {
      toast.error(err.response?.data?.message || 'Erro ao imprimir QR Code');
    },
  });
  ...
  const handleDownload = () => {
    ...
    downloadQRCodeMutation.mutate({ id: generatedQRCode.id, name: generatedQRCode.name });
  };

  const handlePrint = () => {
    if (!generatedQRCode) return;
    printQRCodeMutation.mutate(generatedQRCode.id);
  };
            ...
            <Paper sx={{ p: 3 }}>
              ...
              <Box display="flex" flexDirection="column" gap={2}>
                ...
                <Button
                  ...
                  onClick={handlePrint}
                  fullWidth
                  disabled={printQRCodeMutation.isLoading}
                >
                  ...
                  <div>
                    {index === steps.length - 1 ? (
                      <Button
                        ...
                        sx={{ mt: 1, mr: 1 }}
                      >
                        {generateQRCodeMutation.isLoading ? 'Gerando...' : 'Gerar QR Code'}
                      </Button>
                    ) : (

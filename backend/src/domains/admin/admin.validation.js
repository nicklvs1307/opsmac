...
exports.createUserValidation = [
  ...
  body('roleName').isIn(['owner', 'admin', 'employee', 'super_admin']).withMessage('Papel inválido'),
  body('restaurantId').optional().isUUID().withMessage('ID do restaurante inválido'),
];
...
exports.updateUserValidation = [
  ...
  body('roleName').optional().isIn(['owner', 'admin', 'employee', 'super_admin']).withMessage('Papel inválido'),
  body('restaurantId').optional().isUUID().withMessage('ID do restaurante inválido'),
];
...
exports.createRestaurantWithOwnerValidation = [
  body('restaurantName').trim().notEmpty().withMessage('Nome do restaurante é obrigatório'),
  body('address').trim().notEmpty().withMessage('Endereço do restaurante é obrigatório'),
  body('city').trim().notEmpty().withMessage('Cidade do restaurante é obrigatória'),
  body('state').trim().notEmpty().withMessage('Estado do restaurante é obrigatório'),
  body('ownerName').trim().isLength({ min: 2 }).withMessage('Nome do proprietário deve ter pelo menos 2 caracteres'),
  body('ownerEmail').isEmail().normalizeEmail().withMessage('Email do proprietário deve ter um formato válido'),
  body('ownerPassword').isLength({ min: 6 }).withMessage('Senha do proprietário deve ter pelo menos 6 caracteres'),
];
...
exports.updateRestaurantFeaturesValidation = [
  body('enabledFeatureIds').isArray().withMessage('enabledFeatureIds deve ser um array'),
  body('enabledFeatureIds.*').isInt().withMessage('Cada item em enabledFeatureIds deve ser um inteiro.')
];

...
async function checkCdvModule() {
  try {
    ...
    const submodules = await models.Submodule.findAll({ where: { moduleId: cdvModule.id } });
    if (!submodules.length) {
      console.log('  No submodules found for "cdv".');
    }

    for (const submodule of submodules) {
      console.log(`  Submodule: ${submodule.name} (displayName: ${submodule.displayName})`);
      const features = await models.Feature.findAll({ where: { submoduleId: submodule.id } });
      if (!features.length) {

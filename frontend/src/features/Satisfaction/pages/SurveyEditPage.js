  ...
  questions: yup
    ...
    .of(
      yup.object().shape({
        ...
        options: yup
          .array()
          .of(yup.object().shape({ text: yup.string().required('A opção é obrigatória.') }))
          .optional(),
      })
    )
    .min(1, 'Adicione pelo menos uma pergunta.')
    .required('Adicione pelo menos uma pergunta.'),
});
          ...
          <Controller
            ...
            render={({ field }) => (
              <FormControl fullWidth margin="normal" error={!!errors.rewardId}>
                ...
                <Select {...field} label={t('survey_edit.reward_label')}>
                  <MenuItem value="">
                    <em>{t('common.none')}</em>
                  </MenuItem>
                  {isLoadingRewards ? (

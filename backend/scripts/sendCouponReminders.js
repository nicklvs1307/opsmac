    ...
    for (const coupon of couponsToExpire) {
      try {
        ...
        if (!customer || !restaurant) {
          console.warn(
            `Cupom ${coupon.id}: Cliente ou Restaurante não encontrado. Pulando.`,
          );
          continue;
        }
        ...
        if (!customer.phone) {
          console.warn(
            `Cupom ${coupon.id}: Cliente ${customer.id} não possui número de telefone. Pulando.`,
          );
          continue;
        }
        ...
        if (
          ...
          console.warn(
            `Cupom ${coupon.id}: Credenciais da Evolution API incompletas para o restaurante ${restaurant.id}. Pulando.`,
          );
          continue;
        }

        const couponReminderEnabled =
          restaurant.settings?.whatsappMessages?.couponReminderEnabled;
        const customCouponReminderMessage =
          restaurant.settings?.whatsappMessages?.couponReminderText;
        ...
        if (couponReminderEnabled) {
          ...
          if (whatsappResponse.success) {
            console.log(
              `Lembrete de cupom ${coupon.code} enviado com sucesso para ${customer.phone}`,
            );
            await coupon.update({
              reminderSent: true,
            ...
            console.error(
              `Erro ao enviar lembrete de cupom ${coupon.code} para ${customer.phone}:`,

...
import { useSatisfactionAnalytics } from '@/features/Satisfaction/api/satisfactionService';
import {
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <SatisfactionDateFilters
        ...
        onApplyFilters={() => {
          /* Lógica de aplicação de filtros já está no useSatisfactionAnalytics */
        }}
      />
      ...
      <Grid container spacing={3} mb={4}>
        <Grid item xs={12} md={4}>
          <MetricCard
            title={t('satisfaction_overview.total_responses')}
            value={totalResponses || 0}
          />
        </Grid>
        <Grid item xs={12} md={4}>
          <MetricCard
            title={t('satisfaction_overview.average_nps')}
            value={averageNps?.toFixed(1) || 0}
          />
        </Grid>
        <Grid item xs={12} md={4}>
          <MetricCard
            title={t('satisfaction_overview.average_csat')}
            value={averageCsat?.toFixed(1) || 0}
          />
        </Grid>
      </Grid>

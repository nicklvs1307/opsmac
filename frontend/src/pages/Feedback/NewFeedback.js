...
const NewFeedback = () => {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      toast.error(t('new_feedback.error_loading_initial_data'));
    } finally {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      toast.error(err.response?.data?.message || t('new_feedback.create_error'));
    } finally {
      setLoading(false);
    }
  };
            ...
            <Grid item xs={12}>
              <Box display="flex" gap={2} justifyContent="flex-end">
                ...
                <Button
                  ...
                  disabled={loading}
                >
                  {loading ? t('new_feedback.saving_button') : t('new_feedback.save_button')}
                </Button>
              </Box>
            </Grid>
          </Grid>

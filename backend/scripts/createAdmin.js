...
const createAdminUser = async () => {
  ...
  try {
    ...
    if (!superAdminRole) {
      throw new Error(
        "Função super_admin não encontrada. Certifique-se de que os seeds foram executados.",
      );
    }
    ...
    if (existingAdmin) {
      ...
      console.log(
        `Detalhes: ID: ${newAdmin.id}, Email: ${newAdmin.email}, RoleId: ${newAdmin.roleId}`,
      );
      console.log(
        "Lembre-se de alterar a senha padrão se você não a definiu via variáveis de ambiente.",
      );
    }
  } catch (error) {

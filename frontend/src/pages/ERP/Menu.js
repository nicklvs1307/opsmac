...
const api = {
  ...
  update: (key, { id, ...data }) =>
    axiosInstance.put(`/${key}/${id}`, data).then((res) => res.data),
  delete: (key, id) => axiosInstance.delete(`/${key}/${id}`).then((res) => res.data),
  toggleStatus: (key, id) =>
    axiosInstance.patch(`/${key}/${id}/toggle-status`).then((res) => res.data),
};
...
const CategoriesTab = () => {
  ...
  const createMutation = useMutation(
    (data) => api.create('api/categories', data),
    mutationOptions('criada', 'Categoria')
  );
  const updateMutation = useMutation(
    (data) => api.update('api/categories', data),
    mutationOptions('atualizada', 'Categoria')
  );
  const deleteMutation = useMutation(
    (id) => api.delete('api/categories', id),
    mutationOptions('excluída', 'Categoria')
  );

  const toggleStatusMutation = useMutation((id) => api.toggleStatus('api/categories', id), {
    onSuccess: () => {
          ...
          <tbody>
            ...
            {categories?.map((category) => (
              <tr key={category.id}>
                ...
                <td>
                  <button
                    className={`action-btn toggle-status-btn ${category.is_active ? 'inactive' : 'active'}`}
                  ...
                  <button className="action-btn edit-btn" onClick={() => openModal(category)}>
                    <Icon name="edit" />
                  </button>
                  <button
...
const ProductsTab = () => {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useQuery(['products', restaurantId], () => api.fetch('api/products', restaurantId), {
  ...
  const createMutation = useMutation(
    (data) => api.create('api/products', data),
    mutationOptions('criado')
  );
  const updateMutation = useMutation(
    (data) => api.update('api/products', data),
  ...
  const toggleStatusMutation = useMutation((id) => api.toggleStatus('api/products', id), {
    onSuccess: () => {
          ...
          <tbody>
            ...
            {products?.map((product) => (
              <tr key={product.id}>
                ...
                <td>
                  <button
                    className={`action-btn toggle-status-btn ${product.is_active ? 'inactive' : 'active'}`}
                  ...
                  <button className="action-btn edit-btn" onClick={() => openModal(product)}>
                    <Icon name="edit" />
                  </button>
                  <button
                    className="action-btn delete-btn"
...
const AddonsTab = () => {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useQuery(['addons', restaurantId], () => api.fetch('api/addons', restaurantId), {
    enabled: !!restaurantId,
  });
  ...
  const createMutation = useMutation(
    (data) => api.create('api/addons', data),
    mutationOptions('criado')
  );
  const updateMutation = useMutation(
    (data) => api.update('api/addons', data),
    mutationOptions('atualizado')
  );
  const deleteMutation = useMutation(
    (id) => api.delete('api/addons', id),
    mutationOptions('excluído')
  );

  const toggleStatusMutation = useMutation((id) => api.toggleStatus('api/addons', id), {
    onSuccess: () => {
          ...
          <tbody>
            ...
            {addons?.map((addon) => (
              <tr key={addon.id}>
                ...
                <td>
                  <span className={`status ${addon.is_active ? 'active' : 'inactive'}`}>
                    {addon.is_active ? 'Ativo' : 'Inativo'}
                  </span>
                </td>
                <td>
                  <button
                    className={`action-btn toggle-status-btn ${addon.is_active ? 'inactive' : 'active'}`}
                  ...
                  <button className="action-btn edit-btn" onClick={() => openModal(addon)}>
                    <Icon name="edit" />
                  </button>
                  <button className="action-btn delete-btn" onClick={() => handleDelete(addon.id)}>
                    <Icon name="trash" />
                  </button>
                </td>
              </tr>

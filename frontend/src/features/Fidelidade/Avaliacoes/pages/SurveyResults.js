  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Grid container spacing={3} sx={{ mt: 2 }}>
        ...
        {ratingsAverage !== null && (
          <Grid item xs={12} md={4}>
            <MetricCard title={t('survey_results.ratings_average')} value={ratingsAverage.toFixed(2)} />
          </Grid>
        )}
      </Grid>
      ...
      {Object.keys(npsCriteriaScores).length > 0 && (
        <Box sx={{ mt: 5 }}>
          <NpsScoresByCriterion npsMetricsPerCriterion={Object.values(npsCriteriaScores).map((scores, index) => ({
            id: Object.keys(npsCriteriaScores)[index], // Adicionar ID para key
            name: survey.questions.find((q) => q.npsCriterion?.id === Object.keys(npsCriteriaScores)[index])?.npsCriterion?.name || `Critério ID: ${Object.keys(npsCriteriaScores)[index]}`,

...
const PublicDineInMenu = () => {
  ...
  const orderMutation = useCreateDineInOrder();
      ...
      startSessionMutation.mutate(
        { restaurantSlug, tableNumber },
        {
          onSuccess: (data) => {
            ...
            localStorage.setItem(`tableSession_${restaurantSlug}_${tableNumber}`, data.sessionId);
          },
        }
      );
    }
  }, [products, sessionId, restaurantSlug, tableNumber, startSessionMutation]);

  const handleCallWaiter = () => {
    if (sessionId) {
      callWaiterMutation.mutate(
        { sessionId, description: waiterCallDescription },
        {
          onSuccess: () => {
  ...
  const productsByCategory = products
    ? products.reduce((acc, product) => {
        const category = product.category || t('public_dine_in_menu.uncategorized');
        if (!acc[category]) {
          acc[category] = [];
        }
        ...
        >
          ...
          >
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
              ...
              >
                {t('public_dine_in_menu.table_number_prefix')} {tableNumber}
              </Typography>
            </Box>
          </Toolbar>
        </AppBar>
          ...
          >
            <Box sx={{ width: '100%' }}>
              ...
              <Button
                ...
                disabled={cartItems.length === 0 || orderMutation.isLoading}
              >
        ...
        >
          ...
          <DialogActions sx={{ padding: '20px' }}>
            ...
            <Button
              ...
              disabled={callWaiterMutation.isLoading}
            >
              {callWaiterMutation.isLoading ? (
                <CircularProgress size={24} />
              ) : (

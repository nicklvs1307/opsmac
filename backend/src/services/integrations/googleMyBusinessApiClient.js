...
class GoogleMyBusinessService {
  ...
  async getCredentials() {
    ...
    if (!restaurant || !restaurant.settings?.integrations?.google_my_business) {
      throw new Error(
        "Credenciais do Google My Business não configuradas para este restaurante.",
      );
    }
  ...
  async initializeOAuthClient() {
    const { google_my_business_client_id, google_my_business_client_secret } =
      await this.getCredentials();
    // TODO: Definir redirect_uri corretamente para o seu ambiente
    const redirectUri =
      process.env.GOOGLE_OAUTH_REDIRECT_URI ||
      "http://localhost:5000/api/google-my-business/oauth2callback";
    ...
    if (restaurant.settings?.integrations?.google_my_business?.tokens) {
      this.oauth2Client.setCredentials(
        restaurant.settings.integrations.google_my_business.tokens,
      );
    }
  }
  ...
  getAuthUrl() {
    if (!this.oauth2Client) {
      throw new Error(
        "OAuth2 client not initialized. Call initializeOAuthClient first.",
      );
    }
    const scopes = [
  ...
  async getTokensAndSave(code) {
    ...
    currentSettings.integrations.google_my_business =
      currentSettings.integrations.google_my_business || {};
  ...
  async getLocations() {
    ...
    const business_account = google.mybusinessaccountmanagement({
      version: "v1",

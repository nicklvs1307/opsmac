module.exports = (db) => {
  ...
  const createAddon = async (req, res, next) => {
    ...
    await auditService.log(req.user, restaurantId, 'ADDON_CREATED', `Addon:${newAddon.id}`, { name, price });
    res.json(newAddon);
  };
  ...
  const updateAddon = async (req, res, next) => {
    ...
    await auditService.log(req.user, req.context.restaurantId, 'ADDON_UPDATED', `Addon:${updatedAddon.id}`, { name, price });
  ...
  const deleteAddon = async (req, res, next) => {
    ...
    await auditService.log(req.user, req.context.restaurantId, 'ADDON_DELETED', `Addon:${id}`, {});

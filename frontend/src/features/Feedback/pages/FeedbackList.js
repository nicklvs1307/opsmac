...
import { useGetFeedbacks, useReplyFeedback, useDeleteFeedback } from './api/feedbackService';
...
const FeedbackList = () => {
  ...
  const { data, isLoading, isError, error } = useGetFeedbacks(restaurantId, queryFilters, page);
  ...
  const submitReply = () => {
    ...
    replyMutation.mutate(
      { feedbackId: selectedFeedback.id, replyText },
      {
        onSuccess: () => {
    ...
    <Box>
      ...
      {isError && (
        <Alert severity="error" sx={{ mb: 2 }}>
          {error.message || t('feedback_list.error_loading')}
        </Alert>
      )}

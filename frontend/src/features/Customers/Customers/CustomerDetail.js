...
const CustomerDetail = () => {
  ...
  const resetVisitsMutation = useResetCustomerVisits();
  const clearCheckinsMutation = useClearCustomerCheckins();
  ...
  const handleClearCheckins = () => {
    clearCheckinsMutation.mutate(id, {
      onSuccess: () => {
    ...
    return (
      <Paper sx={{ p: 4, mt: 5, textAlign: 'center' }}>
        <Typography variant="h6" color="error">
          {error.message || 'Erro ao carregar detalhes do cliente.'}
        </Typography>
          ...
          <Card elevation={3}>
            <CardContent>
              ...
              <Typography variant="body1" gutterBottom>
                ...
                Nascimento:{' '}
                {customer.birth_date
                  ? format(new Date(customer.birth_date), 'yyyy-MM-dd', { locale: ptBR })
                  : 'N/A'}
              </Typography>
              <Typography variant="body1" gutterBottom>

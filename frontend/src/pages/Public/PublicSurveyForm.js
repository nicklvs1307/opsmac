...
const PublicSurveyForm = () => {
  ...
  const secondaryColor =
    restaurantData?.settings?.survey_program_settings?.secondary_color || '#2C3E50';
  ...
  const backgroundColor =
    restaurantData?.settings?.survey_program_settings?.background_color || '#F8F9FA';
  const backgroundImageUrl =
    restaurantData?.settings?.survey_program_settings?.background_image_url || null;
  ...
  useEffect(() => {
    const fetchSurvey = async () => {
      try {
        ...
        const errorMessage =
          err.response?.data?.msg || err.message || t('public_survey.survey_not_found_or_inactive');
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axiosInstance.post(
        `/public/surveys/${survey.slug}/responses`,
        payload
      );
      ...
      if (customerId && reward) {
        ...
        // Check for reward object on survey
  ...
  const handleCustomerIdentified = async (newCustomerId) => {
    try {
      const response = await axiosInstance.patch(
        `/public/surveys/responses/${anonymousResponseId}/link-customer`,
        {

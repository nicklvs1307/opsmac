 ...
 *     tags: [Auth]
 *     description: Registra um novo usuário no sistema. Se o papel for 'owner', um restaurante associado é criado.
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
 ...
 *                 type: string
 *                 format: password
 *                 description: Senha do usuário (mínimo 6 caracteres, com maiúscula, minúscula e número).
 *                 example: Senha@123
 *               phone:
 *                 type: string
 ...
 *                 description: Papel do usuário no sistema. Padrão é 'owner'.
 *                 example: owner
 *     responses:
 *       201:
...
/* router.post('/register', authLimiter, registerValidation, async (req, res) => {
  try {
    ...
    const { name, email, password, phone, role = 'owner' } = req.body;
    ...
    // Se o usuário for do tipo 'owner', criar um restaurante associado
    let restaurant = null;
    if (role === 'owner') {
      restaurant = await models.Restaurant.create({
    ...
    const userWithRestaurant = user.toJSON();
    userWithRestaurant.restaurants = restaurant ? [restaurant.toJSON()] : [];

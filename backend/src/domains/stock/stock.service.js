...
exports.getDashboardData = async (restaurantId) => {
  ...
  const totalProducts = await models.Product.count({ where: { restaurant_id: restaurantId } });
...
exports.getAllStocks = async (restaurantId) => {
  const products = await models.Product.findAll({
    where: { restaurant_id: restaurantId },
    attributes: ['id', 'name', 'current_stock', 'min_stock_level', 'unit_of_measure'],
    order: [['name', 'ASC']]
  });
...
exports.createStockMovement = async (restaurantId, movementData) => {
  const { product_id, ingredient_id, quantity, type, notes } = movementData;

  if (!product_id && !ingredient_id) {
    throw new BadRequestError('É necessário informar um produto ou um ingrediente.');
  }
  ...
  if (!item || item.restaurant_id !== restaurantId) {
    throw new NotFoundError('Item não encontrado ou não pertence ao seu restaurante.');
  }

  const newStock = item.current_stock + (type === 'entry' ? quantity : -quantity);
  if (newStock < 0) {
...
exports.getStockHistory = async (restaurantId, productId) => {

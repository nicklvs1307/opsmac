...
export default (db) => {
  ...
  const handleOrderCreated = async (orderData) => {
    ...
    if (!localRestaurant) {
      console.warn(
        `Restaurante com ID ${restaurantId} do Delivery Much não encontrado no sistema.`,
      );
      return;
    }
  ...
  const handleOrderUpdated = async (orderData) => {
    const order = await models.Order.findOne({
      where: { externalOrderId: orderData.id, platform: "delivery_much" },
    });
    if (order) {
      await order.update({ status: orderData.status });
    } else {
      console.warn(
        `Pedido Delivery Much ${orderData.id} não encontrado para atualização de status.`,
      );
    }
  };
  ...
  const checkDeliveryMuchModuleEnabled = async (
    ...
    if (!restaurantId) {
      throw new BadRequestError(
        "ID do restaurante ausente. Não é possível verificar o módulo.",
      );
    }

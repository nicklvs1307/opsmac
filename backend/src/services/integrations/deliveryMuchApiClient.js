...
class DeliveryMuchService {
  ...
  async getCredentials() {
    ...
    if (!restaurant || !restaurant.settings?.integrations?.delivery_much) {
      throw new Error('Credenciais do Delivery Much não configuradas para este restaurante.');
    }
    const { delivery_much_client_id, delivery_much_client_secret, delivery_much_username, delivery_much_password } = restaurant.settings.integrations.delivery_much;
    ...
    return { delivery_much_client_id, delivery_much_client_secret, delivery_much_username, delivery_much_password };
  }

  async authenticate() {
    await this.getCredentials();
    try {
      const response = await axios.post(`${DELIVERY_MUCH_API_BASE_URL}/oauth/token`, {
        grant_type: 'password',
  ...
  async getOrders(status = 'pending') {
    try {
      ...
      console.error('Error fetching Delivery Much orders:', error.response?.data || error.message);
  ...
  async updateOrderStatus(orderId, newStatus) {
    try {
      const headers = await this.getHeaders();
      const response = await axios.put(`${DELIVERY_MUCH_API_BASE_URL}/orders/${orderId}/status`, {
        status: newStatus,
      }, {
        headers,
      });
      ...
      console.error(`Error updating Delivery Much order ${orderId} status to ${newStatus}:`, error.response?.data || error.message);
      throw new Error(`Falha ao atualizar status do pedido ${orderId} no Delivery Much.`);
    }
  }

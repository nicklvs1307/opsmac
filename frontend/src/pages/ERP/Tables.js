...
const createTable = (table) => {
  return axiosInstance.post('/api/tables', table);
};

const updateTable = ({ id, ...table }) => {
  return axiosInstance.put(`/api/tables/${id}`, table);
};

const deleteTable = (id) => {
  return axiosInstance.delete(`/api/tables/${id}`);
};

const generateQrCode = (id) => {
  return axiosInstance.post(`/api/tables/${id}/generate-qr`);
};
...
const Tables = () => {
  ...
  const createMutation = useMutation(createTable, {
    onSuccess: () => {
  ...
  const updateMutation = useMutation(updateTable, {
    onSuccess: () => {
      queryClient.invalidateQueries('tables');
      setOpenFormDialog(false);
  ...
  const generateQrMutation = useMutation(generateQrCode, {
    onSuccess: (data) => {

...
} from '@/features/ERP/Pdv/api/pdvService';
...
const Pdv = () => {
  ...
  const handlePlaceOrder = () => {
    ...
    if (orderType === 'dine_in') {
      if (!selectedTable) {
        toast.error(t('pdv.please_select_table_first'));
  ...
  const handleStatusChange = (orderId, newStatus) => {
    updateOrderStatusMutation.mutate({ restaurantId, orderId, status: newStatus });
  };
  ...
  const handleSaveWithdrawal = (data) => {
    createWithdrawalMutation.mutate({ restaurantId, ...data });
    setWithdrawalModalOpen(false);
  };

  const handleSaveReinforcement = (data) => {
    createReinforcementMutation.mutate({ restaurantId, ...data });
  ...
  useEffect(() => {
    if (!isMobile) {
      setShowOrderSectionMobile(true);
    }
  }, [isMobile]);
  ...
  if (
    isErrorOrders ||
    isErrorRestaurantStatus ||
    isErrorProducts ||
    isErrorCategories ||
    isErrorTables ||
    isErrorCashRegisterSession ||
    isErrorCashOrders
  ) {
    return (
      <Box display="flex" justifyContent="center" alignItems="center" minHeight="100vh">
          ...
          <div className="flex-grow overflow-hidden relative flex flex-col">
                      ...
                      cartItems.map((item) => (
                        ...
                        >
                          ...
                          <div className="flex items-center">
                            <button
                              className="w-6 h-6 rounded-full border-none bg-gray-100 text-gray-800 cursor-pointer flex items-center justify-center transition-all duration-300 hover:bg-gray-100"
            ...
            >
              ...
              <div className="grid grid-cols-auto-fill-minmax-200 gap-5 mb-8">
                ...
                <StatCard
                  icon={<CheckCircleIcon />}
                  title={t('pdv.completed_orders')}
                  value={completedOrdersCount}
                  color="success"
                />
                <StatCard
                  icon={<FireIcon />}
                  title={t('pdv.in_preparation')}
                  value={inPreparationOrdersCount}
                  color="warning"
                />
                <StatCard
                  icon={<PaymentsIcon />}

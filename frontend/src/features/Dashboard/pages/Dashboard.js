...
import { Box, Grid, Typography, Paper, CircularProgress, Alert, Avatar } from '@mui/material';
import {
...
import { useDashboardOverview } from '@/features/Dashboard/api/dashboardService'; // Import the react-query hook

const Dashboard = () => {
  const { user } = useAuth();
  const { t } = useTranslation();

  const [period] = useState('30d'); // Keep period state
  ...
  const {
    ...
    enabled: !!restaurantId, // Only fetch data if restaurantId is available
  });

  // Handle owner with no restaurant assigned
  if (user?.role?.name === 'owner' && (!user.restaurants || user.restaurants.length === 0)) {
    return (
      <Box textAlign="center" p={5}>
        <Typography variant="h4" gutterBottom>
          {t('dashboard.welcome_owner_title')}
        </Typography>
        <Typography variant="body1">{t('dashboard.welcome_owner_message')}</Typography>
      </Box>
    );
  }

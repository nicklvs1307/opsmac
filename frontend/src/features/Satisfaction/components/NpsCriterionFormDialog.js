...
const NpsCriterionFormDialog = ({
    ...
    if (criterion) {
      ...
      updateMutation.mutate(
        { id: criterion.id, name: data.name },
        {
          ...
          onError: (err) => {
            toast.error(
              t('satisfaction_settings.criterion_update_error', { message: err.message })
            );
          },
        }
      );
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm">
      <DialogTitle>
        {criterion
          ? t('satisfaction_settings.edit_criterion_title')
          : t('satisfaction_settings.add_criterion_title')}
      </DialogTitle>

...
const OAuth2 = google.auth.OAuth2;
  ...
  const getLocationsInternal = async (restaurantId) => {
    ...
    if (!account) {
      throw new NotFoundError(
        "Nenhuma conta do Google My Business encontrada.",
      );
    }
  ...
  const oauth2Callback = async (code, state) => {
    ...
    return (
      ...
      "/integrations?status=success&integration=google-my_business"
    );
  };
  ...
  const replyToReview = async (
    ...
    return replyToReviewInternal(
      restaurantId,
      `locations/${locationName}/reviews/${reviewName}`,
      comment,
    );
  };

module.exports = (db) => {
  ...
  const getProductById = async (req, res) => {
    const restaurantId = req.context.restaurantId;
    const product = await productCrudService.getProductById(
      req.params.id,
      restaurantId,
    );
    res.json(product);
  ...
  const updateProduct = async (req, res) => {
    handleValidationErrors(req);
    const restaurantId = req.context.restaurantId;
    const product = await productCrudService.updateProduct(
      req.params.id,
      restaurantId,
      req.body,
    );
  ...
  const toggleProductStatus = async (req, res) => {
    const restaurantId = req.context.restaurantId;
    const product = await productCrudService.toggleProductStatus(
      req.params.id,
      restaurantId,
    );
    await auditService.log(

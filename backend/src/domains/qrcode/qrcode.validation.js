...
exports.createQRCodeValidation = [
  body('table_number').isInt({ min: 1 }).withMessage('Número da mesa deve ser um número positivo'),
  body('location_description').optional().trim().isLength({ max: 200 }).withMessage('Descrição da localização deve ter no máximo 200 caracteres'),
  body('capacity').optional().isInt({ min: 1 }).withMessage('Capacidade deve ser um número positivo'),
  body('area').optional().trim().isLength({ max: 50 }).withMessage('Área deve ter no máximo 50 caracteres'),
];
...
exports.updateQRCodeValidation = [
  ...
  body('capacity').optional().isInt({ min: 1 }).withMessage('Capacidade deve ser um número positivo'),
  body('area').optional().trim().isLength({ max: 50 }).withMessage('Área deve ter no máximo 50 caracteres'),
  body('is_active').optional().isBoolean().withMessage('Status ativo deve ser verdadeiro ou falso'),
  body('status').optional().isIn(['available', 'occupied', 'reserved', 'maintenance', 'inactive']).withMessage('Status deve ser: available, occupied, reserved, maintenance ou inactive')
];
...
exports.cloneQRCodeValidation = [
  body('table_numbers').isArray({ min: 1 }).withMessage('Números das mesas deve ser um array com pelo menos um item'),
  body('table_numbers.*').isInt({ min: 1 }).withMessage('Cada número de mesa deve ser um número positivo')
];
...
exports.listQRCodesValidation = [
  ...
  query('qr_type').optional({ checkFalsy: true }).isIn(['feedback', 'checkin', 'menu']),
  query('is_generic').optional().isBoolean().withMessage('is_generic deve ser verdadeiro ou falso')
];

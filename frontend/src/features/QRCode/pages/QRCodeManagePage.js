...
import {
  useRestaurantQRCodes,
  useQRCodeAnalytics,
  useDownloadQRCode,
  usePrintQRCode,
  useDeleteQRCode,
} from '../api/qrCodeQueries';
...
const QRCodeManage = () => {
  ...
  const {
    ...
    isError: isErrorQRCodes,
    error: qrCodesError,
  } = useRestaurantQRCodes(restaurantId, page, 12, filters, {
    onError: (error) => {
      toast.error(error.response?.data?.message || 'Erro ao carregar QR codes.');
    },
  });
  ...
  } = useQRCodeAnalytics(selectedQRCode?.id, {
    enabled: analyticsDialog && !!selectedQRCode?.id, // Only fetch when dialog is open and QR code is selected
    onError: (error) => {
      toast.error(error.response?.data?.message || 'Erro ao carregar analytics.');
    },
  });
  ...
  const printQRCodeMutation = usePrintQRCode({
    onError: (error) => {
  ...
  const handleDownload = () => {
    ...
    downloadQRCodeMutation.mutate({ id: selectedQRCode.id, name: selectedQRCode.name });
    handleMenuClose();
  };

  const handlePrint = () => {
    if (!selectedQRCode) return;
    printQRCodeMutation.mutate(selectedQRCode.id);
    handleMenuClose();
  };
  ...
  const confirmDelete = () => {
    if (!selectedQRCode) return;
    deleteQRCodeMutation.mutate(selectedQRCode.id);
  };
  ...
  if (isErrorQRCodes) {
    return (
      <Alert severity="error" sx={{ mb: 2 }}>
        {qrCodesError?.message || 'Erro ao carregar QR codes.'}
      </Alert>
    );
  }

  const qrCodes = qrCodesData?.qrcodes || [];
  const totalPages = qrCodesData?.pagination?.total_pages || 1;
  ...
  return (
    <Box>
      ...
      {qrCodes.length === 0 && !isLoadingQRCodes && (
      ...
      <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={handleMenuClose}>
        ...
        <MenuItem onClick={handleDownload} disabled={downloadQRCodeMutation.isLoading}>
          {downloadQRCodeMutation.isLoading ? (
        ...
        <MenuItem onClick={handlePrint} disabled={printQRCodeMutation.isLoading}>
          {printQRCodeMutation.isLoading ? (
            <CircularProgress size={20} />
          ) : (
            <PrintIcon sx={{ mr: 1 }} />
          )}
        ...
        <MenuItem
          ...
          sx={{ color: 'error.main' }}
          disabled={deleteQRCodeMutation.isLoading}
        >
          {deleteQRCodeMutation.isLoading ? (
            <CircularProgress size={20} />
          ) : (
            <DeleteIcon sx={{ mr: 1 }} />
          )}
      ...
      <Dialog open={viewDialog} onClose={() => setViewDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogActions>
          ...
          <Button
            ...
            variant="contained"
            disabled={downloadQRCodeMutation.isLoading}
          >
            {downloadQRCodeMutation.isLoading ? <CircularProgress size={20} /> : 'Baixar'}
          </Button>
        </DialogActions>
      </Dialog>
        ...
        <DialogContent>
          {isLoadingAnalytics ? (
            ...
            <Alert severity="error">
              {analyticsError?.message || 'Erro ao carregar analytics.'}
            </Alert>
          ) : (
            analytics && (
              <Grid container spacing={2}>
      ...
      <Dialog open={deleteDialog} onClose={() => setDeleteDialog(false)}>
        ...
        <DialogActions>
          ...
          <Button
            ...
            disabled={deleteQRCodeMutation.isLoading}
          >
            {deleteQRCodeMutation.isLoading ? <CircularProgress size={20} /> : 'Excluir'}
          </Button>
        </DialogActions>
      </Dialog>
    </Box>

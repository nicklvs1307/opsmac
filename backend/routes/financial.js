  ...
  async (req, res) => {
    ...
    try {
      ...
      const cashMovements = await models.CashRegisterMovement.findAll({
        where: {
          '$session.restaurant_id: restaurantId, // Access through associated session
          createdAt: {
            [models.Sequelize.Op.gte]: startDate,
            [models.Sequelize.Op.lte]: endDate,
          },
        ...
        include: [{
          ...
          as: 'session',
          attributes: [] // Don't fetch session attributes, just use for where clause
        }],
  ...
  async (req, res) => {
    ...
    try {
      const salesData = await models.Order.findAll({
        ...
        order: [[models.Sequelize.col('total_sales'), 'DESC']],
      });

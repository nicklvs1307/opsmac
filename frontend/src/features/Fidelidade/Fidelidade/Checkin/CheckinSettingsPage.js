...
const CheckinSettingsPage = () => {
  ...
  const updateSettingsMutation = useUpdateCheckinSettings(restaurantId);
  const updateProfileMutation = useUpdateRestaurantProfile(restaurantId);
  ...
  const onSave = (data) => {
    ...
    updateSettingsMutation.mutate({ checkin_program_settings: settings });
    updateProfileMutation.mutate({ slug: restaurant_slug });
  };

...
module.exports = (db) => {
    ...
    router.post('/rewards/:rewardId/spin-wheel', checkRestaurantOwnership, requirePermission('fidelity:coupons:raffle', 'create'), asyncHandler(dashboardController.spinWheel));
    router.get('/benchmarking', checkRestaurantOwnership, requirePermission('fidelity:general:benchmarking', 'read'), asyncHandler(dashboardController.getBenchmarkingData));
    router.get('/reports/:reportType', checkRestaurantOwnership, requirePermission('fidelity:reports', 'read'), asyncHandler(dashboardController.getReport));

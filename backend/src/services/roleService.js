...
class RoleService {
  ...
  async deleteRole(roleId, restaurantId) {
    ...
    const usersWithRole = await models.UserRole.count({ where: { roleId: roleId } });
    if (usersWithRole > 0) {
      throw new Error('Conflict: Role is currently assigned to users and cannot be deleted.');
    }
    await role.destroy();
  }
  ...
  async assignUserRole(userId, restaurantId, roleId) {
    ...
    if (!user || !role) {
      throw new Error('User or Role not found or does not belong to this restaurant.');
    }

    const existingUserRole = await models.UserRole.findOne({ where: { userId: userId, restaurantId, roleId } });
    if (existingUserRole) {
  ...
  async removeUserRole(userId, restaurantId, roleId) {
    const result = await models.UserRole.destroy({ where: { userId, restaurantId, roleId } });
    if (result === 0) {
  ...
  async setUserPermissionOverrides(userId, restaurantId, overrides) {
    ...
    await models.UserPermissionOverride.destroy({ where: { userId: userId, restaurantId } });

...
const ManageLabelItems = () => {
    ...
    const handleSave = async (updatedItem) => {
        try {
            const response = await apiClient.patch(`/labels/items/${updatedItem.type}/${updatedItem.id}`, updatedItem);
            // Update the item in the local state
            setItems(items.map(item => (item.id === updatedItem.id && item.type === updatedItem.type ? response.data : item)));
            handleCloseModal();
        } catch (err) {

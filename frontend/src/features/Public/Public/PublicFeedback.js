...
import { useCreatePublicFeedback } from '../../Feedback/api/feedbackService';
import { useGetPublicQRCode } from '../../Feedback/api/feedbackService';
...
const PublicFeedback = () => {
  ...
  const { data: qrCodeData, isLoading, isError, error } = useGetPublicQRCode(qrId);
  ...
  useEffect(() => {
    if (qrCodeData) {
      if (!qrCodeData.qrCode.is_active) {
        return;
      }
      if (qrCodeData.qrCode.expires_at && new Date(qrCodeData.qrCode.expires_at) < new Date()) {
        return;
      }
      if (qrCodeData.qrCode.table_number) {
        setValue('table_number', qrCodeData.qrCode.table_number);
      }
    }
  }, [qrCodeData, setValue]);
  ...
  const onSubmit = (data) => {
    const feedbackData = {
      ...data,
      qr_code_id: qrCodeData.qrCode.id,
      restaurant_id: qrCodeData.restaurant.id,
      source: 'qr_code',
    };

    createPublicFeedbackMutation.mutate(feedbackData, {
      onSuccess: () => {
        setSubmitted(true);
        setTimeout(() => {
          navigate('/obrigado', {
            state: {
              restaurant: qrCodeData.restaurant.name,
              rating: data.rating,
            },
          });
        }, 3000);
          ...
          <CardContent sx={{ p: { xs: 3, md: 4 } }}>
            <Box display="flex" alignItems="center" gap={3} flexWrap={{ xs: 'wrap', sm: 'nowrap' }}>
              ...
              <Box>
                ...
                >
                  <Chip
                    label={qrCodeData?.restaurant?.cuisine_type}
                    size="small"
                    sx={{
                      bgcolor: 'rgba(255,255,255,0.2)',
                      color: 'white',
                ...
                {qrCodeData?.qrCode?.location && (
                  ...
                  >
                    ...
                    {qrCodeData.qrCode.table_number && (
                      <Chip
                        label={
                          t('public_feedback.table_prefix') + ` ${qrCodeData.qrCode.table_number}`
                        }
                        size="small"
                        sx={{
                        ...
                        <Box>
                          ...
                          >
                            ...
                            <Button
                              ...
                              disabled={createPublicFeedbackMutation.isLoading}
                              size="large"
                              sx={{
                                borderRadius: 2,
                                px: 3,
                                ...
                                boxShadow: '0 4px 10px rgba(25, 118, 210, 0.3)',
                                transition: 'all 0.3s ease',
                                bgcolor: createPublicFeedbackMutation.isLoading
                                  ? 'primary.light'
                                  : 'primary.main',
                                '&:hover': {
                                  boxShadow: '0 6px 15px rgba(25, 118, 210, 0.4)',
                            ...
                            >
                              {createPublicFeedbackMutation.isLoading ? (

...
export default (db) => {
  ...
  const createTechnicalSpecification = async (
    ...
    if (!product) {
      throw new NotFoundError(
        "Produto não encontrado ou não pertence ao seu restaurante.",
      );
    }
    ...
    if (existingTechSpec) {
      throw new BadRequestError(
        "Ficha técnica já existe para este produto. Use PUT para atualizar.",
      );
    }
    ...
    if (ingredients.length !== ingredientIds.length) {
      throw new NotFoundError(
        "Um ou mais ingredientes não encontrados ou não pertencem ao seu restaurante.",
      );
    }
  ...
  const getTechnicalSpecificationByProductId = async (
    productId,
    restaurantId,
  ) => {
    ...
    if (!technicalSpecification) {
      throw new NotFoundError(
        "Ficha técnica não encontrada para este produto.",
      );
    }

    const product = await models.Product.findOne({
      where: {
        id: technicalSpecification.product_id,
        restaurant_id: restaurantId,
      },
    });
    if (!product) {
      throw new ForbiddenError(
        "Acesso negado. Ficha técnica não pertence ao seu restaurante.",
      );
    }
  ...
  const updateTechnicalSpecification = async (
    productId,
    recipe_ingredients,
    restaurantId,
  ) => {
  ...
  const deleteTechnicalSpecification = async (productId, restaurantId) => {
    const technicalSpecification = await models.TechnicalSpecification.findOne({
      where: { product_id: productId },
    });
    if (!technicalSpecification) {

...
module.exports = (qrcodeService) => {
    ...
    return {
        createQRCode: async (req, res, next) => {
            ...
            await auditService.log(req.user, restaurantId, 'QRCODE_CREATED', `QRCode:${qrcode.id}`, { type: qrcode.type, target: qrcode.target_url });
        ...
        updateQRCode: async (req, res, next) => {
            ...
            await auditService.log(req.user, restaurantId, 'QRCODE_UPDATED', `QRCode:${qrcode.id}`, { updatedData: req.body });
        ...
        deleteQRCode: async (req, res, next) => {
            ...
            await auditService.log(req.user, restaurantId, 'QRCODE_DELETED', `QRCode:${req.params.id}`, {});
        ...
        generateQRCodeImage: async (req, res, next) => {
            ...
            await auditService.log(req.user, restaurantId, 'QRCODE_IMAGE_GENERATED', `QRCode:${req.params.id}`, { size, format });
            if (format === 'svg') {
        ...
        generatePrintableQRCode: async (req, res, next) => {
            ...
            await auditService.log(req.user, restaurantId, 'QRCODE_PRINTABLE_GENERATED', `QRCode:${req.params.id}`, { query: req.query });
        ...
        recordScan: async (req, res, next) => {
            ...
            await auditService.log(null, result.restaurant?.id || null, 'QRCODE_SCANNED', `QRCode:${req.params.id}`, { userAgent: req.get('User-Agent'), ipAddress: req.ip });
            res.json({
        ...
        redirectToShortUrl: async (req, res, next) => {
            ...
            await auditService.log(null, null, 'QRCODE_SHORT_URL_REDIRECT', `ShortCode:${req.params.shortCode}`, { userAgent: req.get('User-Agent'), ipAddress: req.ip, redirectUrl });
        ...
        cloneQRCode: async (req, res, next) => {
            ...
            await auditService.log(req.user, restaurantId, 'QRCODE_CLONED', `OriginalQRCode:${req.params.id}`, { clonedCount: clonedQRCodes.length, tableNumbers: req.body.table_numbers });
            res.status(201).json({

...
module.exports = (db) => {
    ...
    return {
        ...
        submitSurveyResponses: async (req, res, next) => {
            ...
            await auditService.log(null, result.restaurantId, 'PUBLIC_SURVEY_SUBMITTED', `Survey:${surveySlug}/Response:${result.responseId}`, { customer_id, answersCount: answers.length });
            res.status(201).json({
        ...
        linkCustomerToResponse: async (req, res, next) => {
            ...
            // No req.user for public routes, so pass null for user
            await auditService.log(null, result.restaurantId, 'PUBLIC_SURVEY_LINKED_CUSTOMER', `Response:${responseId}/Customer:${customer_id}`, {});
            res.json({

...
exports.createTransactionValidation = [
  ...
  body('transaction_date').isISO8601().toDate().withMessage('Transaction date must be a valid date.'),
  ...
  body('is_recurring').isBoolean().withMessage('Is recurring must be a boolean.'),
  body('recurring_interval').optional().isIn(['daily', 'weekly', 'monthly', 'yearly']).withMessage('Recurring interval must be daily, weekly, monthly, or yearly.'),
  body('recurring_ends_at').optional().isISO8601().toDate().withMessage('Recurring ends at must be a valid date.'),
];
...
exports.createPaymentMethodValidation = [
  ...
  body('type').isIn(['cash', 'card', 'pix', 'meal_voucher', 'other']).withMessage('Invalid payment method type.'),
...
exports.updatePaymentMethodValidation = [
  ...
  body('type').optional().isIn(['cash', 'card', 'pix', 'meal_voucher', 'other']).withMessage('Invalid payment method type.'),

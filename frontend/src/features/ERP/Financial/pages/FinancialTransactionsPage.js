...
import { useFinancialTransactions, useFinancialCategories, useCreateFinancialTransaction, useUpdateFinancialTransaction, useDeleteFinancialTransaction, } from '@/features/ERP/Financial/api/financialQueries';
...
const FinancialTransactionsPage = () => {
  ...
  const {
    ...
    isError,
    error,
  } = useFinancialTransactions(`/api/financial/transactions?restaurant_id=${restaurantId}`);
  ...
  const handleSubmit = () => {
    ...
    if (editingTransaction) {
      updateMutation.mutate({ id: editingTransaction.id, fields: transactionData });
    } else {
  ...
  if (isError || isErrorCategories) {
    console.error('Error loading financial data:', error || categoriesError);
    return (
      <Box display="flex" justifyContent="center" alignItems="center" minHeight="80vh">

...
module.exports = (db) => {
    ...
    return {
        ...
        submitFeedback: async (req, res, next) => {
            ...
            await auditService.log(null, restaurant_id, 'PUBLIC_V2_FEEDBACK_SUBMITTED', `Feedback:${newFeedback.id}`, { rating, nps_score });
        ...
        registerCheckin: async (req, res, next) => {
            ...
            // No req.user for public routes, so pass null for user
            await auditService.log(null, restaurant_id, 'PUBLIC_V2_CHECKIN_REGISTERED', `Checkin:${checkin.id}`, { customer_id });
            res.status(201).json({

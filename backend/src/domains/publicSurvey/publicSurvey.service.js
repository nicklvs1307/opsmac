...
exports.getNextSurvey = async (restaurantSlug, customerId) => {
  ...
  const commonSurveyOptions = {
    ...
    attributes: ['id', 'title', 'description', 'type', 'slug', 'rotation_group', 'coupon_validity_days'],
  };
  ...
  if (!targetSurvey) {
    throw new NotFoundError('Nenhuma pesquisa ativa encontrada para este restaurante.');
  }
...
exports.getPublicSurveyBySlugs = async (restaurantSlug, surveySlug) => {
  ...
  if (!survey) {
    throw new NotFoundError('Pesquisa não encontrada ou inativa para este restaurante.');
  }
...
exports.submitSurveyResponses = async (surveySlug, answers, customer_id) => {
  ...
  for (const ans of answers) {
    if (!surveyQuestionIds.includes(ans.question_id)) {
      throw new BadRequestError(`Pergunta com ID ${ans.question_id} não pertence a esta pesquisa.`);
    }
  }
  ...
  for (const ans of answers) {
    ...
    if (question) {
      ...
      if (!isNaN(value)) {
        ...
        } else if (question.question_type === 'ratings' || question.question_type === 'csat') {
...
exports.linkCustomerToResponse = async (responseId, customer_id) => {
  ...
  if (surveyResponse.customer_id) {
    throw new BadRequestError('Esta resposta já está vinculada a um cliente.');
  }

...
import { useGeneratedCouponsReport } from '@/features/Reports/api/financialReportsQueries';
...
const GeneratedCouponsReport = () => {
  ...
  const {
    ...
    isError,
    refetch,
  } = useGeneratedCouponsReport(restaurantId, filters, {
    onError: (error) => {
      toast.error(
        t('reports.error_loading_coupons', { message: error.response?.data?.msg || error.message })
      );
    },
  });
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Typography variant="h4" gutterBottom>
        {t('reports.generated_coupons_title')}
      </Typography>
